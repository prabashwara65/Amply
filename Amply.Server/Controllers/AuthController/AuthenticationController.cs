    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        private async Task<RegisterResponse> RegisterAsync(RegisterRequest request)
        {
            try
            {
                ...
                //user is created 
                //then add user to roal 
                var addUserToRoleResult = await _userManager.AddToRoleAsync(userExists, "USER");
                if (!addUserToRoleResult.Succeeded) return new RegisterResponse { Message = $"Create user succeed but could not add user to role {addUserToRoleResult?.Errors?.First()?.Description}", Success = false };
